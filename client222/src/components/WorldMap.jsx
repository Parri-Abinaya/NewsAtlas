...
// ISO numeric to alpha-2 mapping (partial - common countries)
const numericToAlpha2 = {
  ...
  400:'JO',404:'KE',408:'KP',410:'KR',414:'KW',116:'KH',418:'LA',422:'LB',
  430:'LR',434:'LY',484:'MX',496:'MN',504:'MA',508:'MZ',516:'NA',524:'NP',
  ...
  724:'ES',729:'SD',752:'SE',756:'CH',760:'SY',834:'TZ',764:'TH',792:'TR',
...
function getAlpha2(geo) {
  ...
  return numericToAlpha2[numId] || null;
}
    ...
    <div className="map-container" style={{ height: '500px', position: 'relative' }}>
              ...
              geographies.map(geo => {
                ...
                return (
                  <Geography
                    ...
                    style={{
                      default: {
                        fill: isHovered ? '#00d4aa' : '#1a2332',
                        stroke: '#0d1117',
                        strokeWidth: 0.5,
                        outline: 'none',
                        cursor: 'pointer',
                        filter: isHovered ? 'drop-shadow(0 0 8px rgba(0,212,170,0.6))' : 'none',
      ...
      <div style={{ position: 'absolute', bottom: '1rem', right: '1rem', display: 'flex', gap: '0.5rem' }}>
        <button
          ...
          style={{ background: 'var(--surface2)', border: '1px solid var(--border)', color: 'var(--text)', borderRadius: '6px', width: '32px', height: '32px', cursor: 'pointer', fontSize: '1rem' }}
        >+</button>
        <button
          onClick={() => setPosition(p => ({ ...p, zoom: Math.max(p.zoom / 1.5, 0.8) }))}
          style={{ background: 'var(--surface2)', border: '1px solid var(--border)', color: 'var(--text)', borderRadius: '6px', width: '32px', height: '32px', cursor: 'pointer', fontSize: '1rem' }}>−</button>
        <button
          onClick={() => setPosition({ coordinates: [0, 20], zoom: 1 })}
          style={{ background: 'var(--surface2)', border: '1px solid var(--border)', color: 'var(--text)', borderRadius: '6px', padding: '0 0.6rem', cursor: 'pointer', fontSize: '0.75rem' }}>Reset</button>
      </div>

      <div style={{ position: 'absolute', bottom: '1rem', left: '1rem', fontSize: '0.7rem', color: 'var(--text-subtle)', fontFamily: 'var(--font-mono)' }}>
        Click any country to explore • Scroll to zoom
      </div>
    </div>
  );
}
